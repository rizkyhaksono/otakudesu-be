...
import type { anime, episode_list } from "@/types/anime"

const scrapeSingleAnime = (html: string, baseUrl: string) => {
  const result = createAnimeData(html, baseUrl, getPoster(html), getSynopsis(html), scrapeAnimeEpisodes(html))
  return result
}

const createAnimeData = (html: string, baseUrl: string, poster: string | undefined, synopsis: string | undefined, episode_lists: episode_list[] | undefined): anime | undefined => {
  const $ = load(html)
  const title = $(".infozin .infozingle p:first span").text()?.replace("Judul: ", "")
  const slug = $("link[rel=canonical]").attr("href")?.replace(`${baseUrl}/anime/`, "").replace("/", "")
  ...
  const genres = mapGenres($(".infozin .infozingle p:last span a").toString(), baseUrl)
