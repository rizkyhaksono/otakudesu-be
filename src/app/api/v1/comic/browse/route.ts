...
export async function GET(request: Request) {
  ...
  return apiHandler(
    () => {
      ...
      // Allowlisted rather than passed through: these go straight into an
      // upstream query string, and the set of valid values is small and known.
      const rawType = params.get("type");
      const type = TYPES.includes(rawType ?? "") ? rawType! : undefined;
      const rawSort = params.get("sort");
      const sort = SORTS.includes(rawSort ?? "") ? rawSort! : undefined;

      return comicBrowse({ page, genre, search, type, sort });
    },
